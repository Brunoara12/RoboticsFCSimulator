...
#define V_LOGV(LogCat, Verbosity, Param1) UE_LOG(LogCat, Verbosity, TEXT("%s: %s"), 	*CUR_CLASS_LINE, *FString(Param1))
	...
	/*
		...
		DEBUGP(Location, 1);
	*/
#define DEBUGP(ParamL, ParamT) if(GetWorld()) DrawDebugPoint( 	GetWorld(), 	ParamL, 	20,	FColor::Red, 	false, ParamT);
	...
	*/
#define DEBUGPC(ParamL, ParamC, ParamT) if(GetWorld()) DrawDebugPoint( 	GetWorld(), 	ParamL, 	20,	FColor::ParamC, 	false, ParamT);
...
#define DEBUGSC(ParamL, ParamR, ParamC, ParamT) if(GetWorld()) DrawDebugSphere( 	GetWorld(), 	ParamL, 	ParamR, 	32, 	FColor::ParamC, false, ParamT,0 );
	...
	/*
		Draws a Colored Box at start Location to extent Location for X seconds!
		Ex:
		FVector StartLocation(10,10,8);
		FVector EndLocation(100,100,100);
		DEBUGBOX(StartLocation, CenterLocation, Blue, 4);
	*/
#define DEBUGBOX(ParamS, ParamD, ParamC, ParamT) if(GetWorld()) DrawDebugBox( GetWorld(), ParamS, ParamD, FColor::ParamC, false, ParamT, 2.5);
	/*
